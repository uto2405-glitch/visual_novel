...
최종 갱신: 2026-08-25 (이력 보관 배너 + 액션 아이템 현행화 + "현재 위치" 를 실제 작품 상태로 교체)
...
- 상태: **작품 제작 진행 중.** `project/` 는 더 이상 데모 데이터가 아니다 —
  「두근두근 연애 시뮬레이터」 12장면(1~3화), 승인 7 · 프롬프트 1 · 장면계획 4,
  선택지/분기 장면 2개, 호감도(`dating`) 사용, 후보 이미지 폴더 7개.
  (2026-08-25 확인. 숫자는 그때의 스냅샷이므로, 지금 수치는
  `python tools/advance_scene.py status` 로 본다 — 진행 상태의 단일 출처는 NO_TOKEN_TASKS.md.)
- 아래 "완료됨 / 단계 이력 / 열린 질문" 은 2026-08-24 시점의 이력이다(맨 위 배너 참조).
