...
> 지금 구성과 다르다. 현행은 이렇다: **오케스트레이터는 로컬 LLM 하나뿐이고**
> (`mode: local` · `model: local-qwen`) 원격 API 경로는 없다. 그것이 꺼져 있을 때의 경로는
> **직접 입력(붙여넣기)** 이다. 이미지 생성 기본 엔진은 **로컬 ComfyUI**(무료)이고 MakeFun 은 보조다.
> 아래에 남은 그록 관련 기록(크레딧 충전·`grok-4.6` 기입 등)은 **당시 이력이지 현재 지시가 아니다** —
> 날짜가 붙은 줄은 일부러 고치지 않는다(이력을 고쳐 쓰면 무엇이 언제 참이었는지 사라진다).
...
> 그록 API 크레딧 충전은 **더 이상 할 일이 아니다** — 그 경로 자체가 은퇴했다.
> 로컬 LLM 이 꺼져 있으면 직접 입력(붙여넣기)으로 진행한다(계정·결제 없음).
> 이미지 공급자는 매니페스트 `image_generator` 에 기입돼 있다(기본 ComfyUI · 보조 MakeFun).
