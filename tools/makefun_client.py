...
# 아래 네 숫자는 **아직 측정된 값이 아니다.** 지금 이 코드를 쓰는 시점에 적당해 보인
# 값이고, 근거는 각 줄에 적었다. 첫 실측(VN_MF_PROBE)이 나오면 갈아 끼운다.
#
# 이 주석이 있는 이유: 앞서 pageSize 5 를 상한처럼 박았다가 단가가 실제보다 싸게
# 나올 뻔했다. 그 5는 남이 예시로 든 값이었는데 내가 근거를 안 적고 옮겼다.
# **이유를 못 적는 상수는 대개 근거가 없다는 신호다.**
SPEND_PAGE = 50           # 한 창(생성 한 번)이 50줄을 넘길 일이 없다고 봄. 장수 상한 8 ·
                          #   업스케일/R2 가 끼어도 그 근처다. 근거: 명세의 number_of_images
SPEND_MAX_PAGES = 5       # 50×5=250줄이면 어떤 창이든 덮는다고 봄. 상한이 필요한 이유는
                          #   응답이 늘 꽉 차 오는 고장에서 끝없이 읽지 않기 위해서다
SPEND_RETRY_SEC = 3.0     # **짐작이다.** 과금이 완료 뒤 언제 확정되는지 모른다.
                          #   조사(probe_spend_timing)가 첫 등장 시각을 재면 그 값으로 바꾼다
SPEND_SAMPLE_CAP = 10     # 같은 조건 10건이면 평균이 흔들리지 않는다고 봄(통계가 아니라
                          #   판단이다). 재는 것도 실호출이라 무한정 할 수는 없다
...
PROBE_MAX_SEC = 30        # **짐작이다.** 30초 안에 안 뜨면 '더 늦거나 과금이 없거나' 로
                          #   적는다 — 둘 다 결과이므로 더 오래 붙잡지 않는다
PROBE_STEP_SEC = 1.0      # 1초 간격. 더 촘촘하면 첫 등장 시각이 정확해지지만 조회가
                          #   그만큼 늘어난다(무과금이어도 남의 서버다)
