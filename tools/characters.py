...
얼굴 고정에 대하여 — 지금 이 저장소가 가진 레버는 둘뿐이다:
  ...
  사진을 참고로 등록할 수는 있지만, **지금 엔진은 그 사진을 읽지 않는다** — 그러려면
  ComfyUI 에 IPAdapter 계열 노드가 있어야 하고 지금은 없다.
  그래서 :func:`face_lock_state` 가 그 사실을 사람에게 그대로 말한다. 조용히 사진을
  받아 놓고 안 쓰면, 사람은 얼굴이 흔들릴 때마다 자기 사진이 나쁜 줄 안다.
...
def face_lock_state() -> dict:
    """얼굴을 얼마나 고정할 수 있는가 — **지금 이 PC 기준으로** 솔직하게.

    사진을 받아 놓고 못 쓰면서 말하지 않으면, 사람은 얼굴이 흔들릴 때마다 자기 사진을
    의심한다. 그래서 화면이 이 말을 그대로 보여 준다.
    """
    return {
        "photo_used": False,
        "how": "text",
        "note": ("지금 엔진은 등록한 사진을 보지 못합니다 — 얼굴은 인물 태그와 앵커 문장으로만 "
                 "고정합니다. 사진으로 얼굴을 잡으려면 ComfyUI 에 IPAdapter 계열 노드가 있어야 "
                 "합니다(지금은 없습니다). 사진은 사람이 보고 문장을 적을 참고로 보관합니다."),
    }
