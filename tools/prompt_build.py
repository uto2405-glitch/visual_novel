...
#
# **노을만 두 낱말이다**(실측 · SCENE-012 3시드 × 7안). 시간대 낱말은 ~70토큰 프롬프트의
# 맨 끝 한 토큰이라, 하늘이 넓게 보이는 컷에서는 장소 앵커의 밤 조명 낱말(LOC-003 의
# `low street lamps` · `city skyline across the water`)에 밀려 블루아워로 떨어졌다 —
# 012 의 하늘 파랑 비율 .670 으로 **밤 컷보다 더 파랬다**. 한 낱말을 **앞으로 옮기는 것은
# 오히려 나빴고**(.255 → .309), 더 화려한 문구·반복·네거티브는 과보정이었다(명도 182~214 ·
# 오렌지 .80~.99). `golden hour` **한 낱말만** 더하면 010 · 012 가 3/3 블루아워를 벗어나고
# (.017 / .011) 승인 노을 컷의 색 대역 안에 들어온다(명도 164.0 · 하늘 온기 +36.4).
# 나머지 시간대는 실측이 정상이라 손대지 않는다(밤 007 .611 / 008 .603 = 밤 대역).
TIME_EN = {"밤": "night", "낮": "daytime", "아침": "morning", "저녁": "evening",
           "노을": "sunset, golden hour", "새벽": "dawn", "오후": "afternoon"}
...
STORY_SYSTEM_HEAD = (
    ...
    # 제안 태그: 모델이 '지금 이걸 하면 된다' 고 판단했을 때만 남기는 표시다.
    # 화면(통합 화면)은 이 태그를 버튼으로 바꿔 놓고, 누르는 것은 사람이다 —
    # 태그 자체는 아무것도 실행하지 않으므로, 모델이 형식을 틀려도 버튼이 안 생길 뿐이다.
    # 스튜디오의 스토리 탭은 이 태그를 그냥 글자로 보여 준다(해가 없다).
    "이야기가 장면으로 옮길 만큼 모였다고 판단하면, 답의 마지막 줄에 [제안:조립 N] 을 "
    "덧붙여라(N 은 권하는 장면 수). 특정 장면의 그림을 다시 뽑는 게 좋겠으면 "
    "[제안:그림 SCENE-003] 처럼 덧붙여라. 확신이 없으면 덧붙이지 마라. "
    "태그는 한 줄에 하나씩, 설명 없이 적는다.\n\n")
...
# 작품 문맥을 빼고 쓰는 머리말. '새 대화'는 지금 작품과 무관한 이야기를 시작하는 자리라,
# 인물·장소·스토리라인을 실으면 시키지도 않은 인물이 답에 섞여 나온다(실제로 그랬다:
# 새 대화에 "8살 민수의 등교"만 적었는데 기존 작품의 지혜와 LOC-001~003 이 따라 나왔다).
BLANK_SYSTEM_HEAD = (
    "너는 비주얼 노벨/웹툰 스토리 기획 파트너다. 한국어로 간결하고 구체적으로 답한다.\n"
    "이 대화는 새 이야기다. 사용자가 말하지 않은 인물·장소·설정을 이미 있는 것처럼 "
    "끌어오지 마라. 필요하면 새로 제안하되 그것이 새 제안임을 밝혀라.\n"
    "이야기가 장면으로 옮길 만큼 모였다고 판단하면, 답의 마지막 줄에 [제안:조립 N] 을 "
    "덧붙여라(N 은 권하는 장면 수). 확신이 없으면 덧붙이지 마라.\n")


def story_system_message(with_context: bool = True) -> dict:
    """스토리 챗의 system 메시지.

    with_context=True  : 역할 지시 + 현재 작품 컨텍스트(인물·장소·스토리라인·장면 목록)
    with_context=False : 역할 지시만. 지금 작품과 무관한 새 이야기를 시작할 때 쓴다.
    """
    if not with_context:
        return {"role": "system", "content": BLANK_SYSTEM_HEAD}
