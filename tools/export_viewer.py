...
파일 하나가 곧 디지털 소장본이다. VN 모드(타자기·자동·이어보기·분기·백로그·설정)와
세로 스크롤 웹툰 모드, 화(episode) 구분을 포함한다.
...
UI_TEXT = ("VISUAL NOVEL · 소장본 처음부터 감상 ▸ 이어보기 세로 스크롤로 읽기 자동 기록 스크롤 닫기 "
           "즉시 빠름 보통 느림 탭/Space 진행 ← 이전 Esc VN 모드 지난 대사 설정 글자 크기 "
           "작게 크게 아주 텍스트 속도 화 이동 목록 "
